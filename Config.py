...
# ===通用配置
rule_types = ['8H']
symbol_list = ['ETH-USDT', 'BTC-USDT', 'ETC-USDT']  # 选择交易的股票/币种列表
leverage_rate = 2  # 杠杆倍数
date_start = '2021-04-01'  # 回测开始时间
date_end = '2024-09-29'  # 回测结束时间
c_rate = 5 / 10000  # 手续费，commission fees，默认为万分之5。不同市场手续费的收取方法不同，对结果有影响。
slippage = 1 / 1000  # 滑点 ，可以用百分比，也可以用固定值。建议币用百分比，股票用固定值
min_margin_ratio = 1 / 100  # 最低保证金率，低于就会爆仓
drop_days = 10  # 刚刚上线10天内不交易
symbol_data_path = r''  # 数据路径
...
# ===单策略回测配置
signal_name = 'signal_simple_bolling'  # 策略名称
symbol = ''  # 指定标的
para = [20, 0.6]  # 策略参数
